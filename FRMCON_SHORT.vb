...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FRMCON_SHORT))
        ...
        'btResure
        '
        Me.btResure.Location = New System.Drawing.Point(127, 332)
...
#End Region
    ...
    Private Sub FRMCON_SHORT_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If G_DeptCode = "26.11" Then
            SqlStrShip = SqlStrShip + " and ship_id in (select distinct ship_id from VIEWCON_SHORT) "
        End If
    ...
    Private Sub C1DBGV_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles C1DBGV.DoubleClick
        ...
        If dsShip.Tables(0).DefaultView.Count > 0 Then
            ...
            End If
        End If
        FirmStr = " and Ship_ID=" & Ship_ID & " Order by TradeMark,ID " '设计初始条件  在检索后不改变的条件
        DynaStr = " 2>1 " '设计其它初始条件    在检索后不需要的条件
        SelectStr = " "
        ...
        '初始化打印标志
        ExecSql("update con_short set printout='0' WHERE SHIP_ID='" & Ship_ID & "'")
...
Err:
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Dim i As Integer
        Try
            strsql = "select " & SelStr & " *  from VIEWCON_SHORT where " & Sql
...
Err:
    ...
    Private Sub ToolBar_ButtonClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs) Handles ToolBar.ButtonClick
        Select Case Me.ToolBar.Buttons.IndexOf(e.Button)
            ...
            Case 1
                Try
                    printflag = 1
                    Dim frm As New FRMCON_SHORT_DO
                    frm.ShowDialog()
                    Call myDataGrid(SelectStr, SqlStr)
                Catch ex As Exception
                    MsgBox(ex.Message)
    ...
    Private Sub mnuFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFind.Click
        Dim FrmFind As New FrmFind
        Table_Name = "VIEWCON_SHORT" '可能要修改
        FrmFind.ShowDialog()
    ...
    Private Sub mnuDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuDelete.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmDelete As New FRMCON_SHORT_DELETE   '***********要修改
    ...
    Private Sub mnuEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEdit.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmEdit As New FRMCON_SHORT_EDIT   '***********要修改
            Dim Row As Integer
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            Row = Me.C1DBG.Row
            FrmEdit.ShowDialog()
    ...
    Private Sub mnuAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuAdd.Click
        Dim FrmAdd As New FRMCON_SHORT_ADD   '***********要修改
        If ds.Tables(0).Rows.Count > 0 Then
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
        Else
            ID = -1
    ...
    Private Sub mnuQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuQuery.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmQuery As New FRMCON_SHORT_QUERY   '***********要修改
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            FrmQuery.ShowDialog()
        End If
    End Sub
    ...
    Private Sub mnuPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuPrint.Click
        GL_Print = 1
        Getdata("select * from con_short where SHIP_ID='" & Ship_ID & "' ", dsprint)
        If dsprint.Tables(0).Rows.Count = 0 Then
            Call MakeExcel()
        Else
            Getdata("select * from con_short where SHIP_ID='" & Ship_ID & "' and printout='0' ", dsprint)
            If dsprint.Tables(0).Rows.Count > 0 Then
                Call MakeExcel()
            Else
                MsgBox("打印任务已结束，请双击左侧船名重新打印！")
            End If
        End If
    End Sub
    ...
    Private Sub MakeExcel()
        ...
        Try
            ...
            xlsheet = CType(xlbook.Worksheets("残损溢短单"), Excel.Worksheet)
            ...
            xlsheet.Cells(9, 8) = "'" & ds_sship.Tables(0).Rows(0).Item("VOYAGE")
            xlsheet.Cells(9, 12) = "'" & ds_sship.Tables(0).Rows(0).Item("NATIONALITY_English")
            xlsheet.Cells(9, 15) = "'NO." & ds_sship.Tables(0).Rows(0).Item("berthno")
            If IsDBNull(ds_sship.Tables(0).Rows(0).Item("worktime_begin")) Then
                xlsheet.Cells(10, 4) = "'" & Now().Day.ToString().PadLeft(2, "0") & "-" & Now().Month.ToString().PadLeft(2, "0") & "-" & Now().Year.ToString()
            Else
                xlsheet.Cells(10, 4) = "'" & System.Convert.ToString(Day(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN"))).PadLeft(2, "0") & "-" & System.Convert.ToString(Month(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN"))).PadLeft(2, "0") & "-" & Year(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN"))
            End If
            'xlsheet.Cells(10, 12) = "'" & System.Convert.ToString(Day(Now)).PadLeft(2, "0") & "-" & System.Convert.ToString(Month(Now)).PadLeft(2, "0") & "-" & Year(Now)
            If IsDBNull(ds_sship.Tables(0).Rows(0).Item("worktime_end")) Then
                xlsheet.Cells(10, 12) = "'" & Now().Day.ToString().PadLeft(2, "0") & "-" & Now().Month.ToString().PadLeft(2, "0") & "-" & Now().Year.ToString()
            Else
                xlsheet.Cells(10, 12) = "'" & System.Convert.ToString(Day(ds_sship.Tables(0).Rows(0).Item("worktime_end"))).PadLeft(2, "0") & "-" & System.Convert.ToString(Month(ds_sship.Tables(0).Rows(0).Item("worktime_end"))).PadLeft(2, "0") & "-" & Year(ds_sship.Tables(0).Rows(0).Item("worktime_end"))
            End If

            '分开统计残损、溢、短
            'Dim strshort As String
            Dim dsshort As New DataSet
            Getdata("exec spcon_short_print '" & Ship_ID & "','" & C1DBG.Columns("TradeMark").Text & "'", dsshort)

            If (dsshort.Tables(0).Rows.Count < 11) Then
                For row = 0 To dsshort.Tables(0).Rows.Count - 1
                    xlsheet.Cells(14 + row, 1) = dsshort.Tables(0).Rows(row)("CONTAINER_NO")
                    'xlsheet.Cells(14 + row, 3) = dsshort.Tables(0).Rows(row)("SEALNO")
                Next row
            Else
                For row = 0 To 9
                    xlsheet.Cells(14 + row, 1) = dsshort.Tables(0).Rows(row)("CONTAINER_NO")
                ...
                Next row
            End If
            If (dsshort.Tables(1).Rows.Count < 11) Then
                For row = 0 To dsshort.Tables(1).Rows.Count - 1
                    xlsheet.Cells(14 + row, 5) = dsshort.Tables(1).Rows(row)("CONTAINER_NO")
                    'xlsheet.Cells(14 + row, 7) = dsshort.Tables(1).Rows(row)("SEALNO")
                Next row
            Else
                For row = 0 To 9
                    xlsheet.Cells(14 + row, 5) = dsshort.Tables(1).Rows(row)("CONTAINER_NO")
                ...
                Next row
            End If
            If (dsshort.Tables(2).Rows.Count < 11) Then
                For row = 0 To dsshort.Tables(2).Rows.Count - 1
                    xlsheet.Cells(14 + row, 9) = dsshort.Tables(2).Rows(row)("CONTAINER_NO")
                    'xlsheet.Cells(14 + row,11) = dsshort.Tables(2).Rows(row)("SEALNO")
                    xlsheet.Cells(14 + row, 13) = dsshort.Tables(2).Rows(row)("DAMAGED_CONDITION")
                Next row
            Else
                For row = 0 To 9
                    xlsheet.Cells(14 + row, 9) = dsshort.Tables(2).Rows(row)("CONTAINER_NO")
