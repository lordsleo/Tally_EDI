...
Public Class FRMCON_CERTIFICATE_SUM
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FRMCON_CERTIFICATE_SUM))
        ...
        'ToolBar
        '
        Me.ToolBar.Buttons.AddRange(New System.Windows.Forms.ToolBarButton() {Me.tbbtnSep2, Me.tbbtnPrint, Me.tbbtnExport, Me.tbbtnEDI, Me.tbbtnSep3, Me.tbbtnQuit})
        ...
        Me.Menu = Me.MainMenu
        Me.Name = "FRMCON_CERTIFICATE_SUM"
        Me.Text = "集装箱理货汇总证明"
...
#End Region
    ...
    Private Sub FRMCON_CERTIFICATE_SUM_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub C1DBGV_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles C1DBGV.DoubleClick
        ...
        DS_DO.Reset()
        dw = Getdata("exec SPCon_CertificateSumNew " & Ship_ID & "", DS_DO)
        'If MsgBox("是否生成实际理箱报表？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.Yes Then
        '    dw = Getdata("exec spcon_certificate_of_sum " & Ship_ID & ",0 ", DS_DO)
        'Else
        '    dw = Getdata("exec spcon_certificate_of_sum " & Ship_ID & ",1 ", DS_DO)
        'End If
        'If ds_sship.Tables(0).Rows.Count > 0 Then
        '    trade = ds_sship.Tables(0).Rows(0).Item("s_trade")
        '    If trade = "1" Then
        '        dw = Getdata("exec spcon_certificate_of_sum '" & Ship_ID & "'", DS_DO)
        '    ElseIf trade = "2" Then
        '        dw = Getdata("exec spcon_certificate_of_sum_eng '" & Ship_ID & "'", DS_DO)
        '    End If
        'End If
...
Err:
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Try
            ...
            Next
            Me.C1DBG.Columns("TradeDesc").Caption = "贸别"
            Me.C1DBG.Columns("Type").Caption = "项目"
            Me.C1DBG.Columns("Con20").Caption = "20英尺箱"
            Me.C1DBG.Columns("Con40").Caption = "40英尺箱"
            Call SetColumnWidth()
            Me.C1DBG.Refresh()
        Catch ex As Exception
            MsgBox(ex.Message)
...
Err:
    ...
    Private Sub china_new()
        ...
        Try
            ...
            'If io = 0 Then
            '    xlsheet.Cells(10, 1) = "进口"
            'Else
            '    xlsheet.Cells(10, 1) = "出口"
            'End If

            xlsheet.Cells(8, 11) = "全"
            ...
            Else
                xlsheet.Cells(13, 3) = " " & Year(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN")) & "年 " & Month(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN")) & "月 " & Day(ds_sship.Tables(0).Rows(0).Item("WORKTIME_BEGIN")) & "日"
            End If
            xlsheet.Cells(13, 9) = " " & Year(Now) & "年 " & Month(Now) & "月 " & Day(Now) & "日"
            Dim i As Integer
            Dim blInTrade As Boolean = False
            Dim iStart As Integer = 0
            Dim iEnd As Integer = C1DBG.RowCount
            '检查外贸船带内贸箱
            If C1DBG.Columns("TradeMark").Text = "1" Then
                blInTrade = True
            End If
            '分贸别确定统计数据起止位置
            For i = 0 To C1DBG.RowCount - 1
                If C1DBG.Columns("TradeMark").CellText(i) = "1" Then
                    If blInTrade Then
                        iStart = i
                    Else
                        iEnd = i
            ...
            For i = iStart To iEnd - 1
                Select Case C1DBG.Columns("Seq").CellText(i)
                    Case "1"
                        If C1DBG.Columns("Con20").CellText(i) <> "" Then
                            xlsheet.Cells(20, "F") = C1DBG.Columns("Con20").CellText(i)
                        End If
                        If C1DBG.Columns("Con40").CellText(i) <> "" Then
                            xlsheet.Cells(20, "I") = C1DBG.Columns("Con40").CellText(i)
                        End If
                    Case "2"
                        If C1DBG.Columns("Con20").CellText(i) <> "" Then
                            xlsheet.Cells(23, "F") = C1DBG.Columns("Con20").CellText(i)
                        End If
                        If C1DBG.Columns("Con40").CellText(i) <> "" Then
                            xlsheet.Cells(23, "I") = C1DBG.Columns("Con40").CellText(i)
                        End If
                    Case "3"
                        If C1DBG.Columns("Con20").CellText(i) <> "" Then
                            xlsheet.Cells(26, "F") = C1DBG.Columns("Con20").CellText(i)
                        End If
                        If C1DBG.Columns("Con40").CellText(i) <> "" Then
                            xlsheet.Cells(26, "I") = C1DBG.Columns("Con40").CellText(i)
                        End If
                    Case "4"
                        If C1DBG.Columns("Con20").CellText(i) <> "" Then
                            xlsheet.Cells(27, "F") = C1DBG.Columns("Con20").CellText(i)
                        End If
                        If C1DBG.Columns("Con40").CellText(i) <> "" Then
                            xlsheet.Cells(27, "I") = C1DBG.Columns("Con40").CellText(i)
                        End If
                    Case "5"
                        If C1DBG.Columns("Con20").CellText(i) <> "" Then
                            xlsheet.Cells(28, "F") = C1DBG.Columns("Con20").CellText(i)
                        End If
                        If C1DBG.Columns("Con40").CellText(i) <> "" Then
                            xlsheet.Cells(28, "I") = C1DBG.Columns("Con40").CellText(i)
                        End If
                End Select
            Next
            'If DS_DO.Tables(0).Rows(0)(1) > 0 Then
            '    xlsheet.Cells(20, 6) = DS_DO.Tables(0).Rows(0)(1)
            'End If
            'If DS_DO.Tables(0).Rows(0)(2) > 0 Then
            '    xlsheet.Cells(20, 9) = DS_DO.Tables(0).Rows(0)(2)
            'End If
            'If DS_DO.Tables(0).Rows(1)(1) > 0 Then
            '    xlsheet.Cells(23, 6) = DS_DO.Tables(0).Rows(1)(1)
            'End If
            'If DS_DO.Tables(0).Rows(1)(2) > 0 Then
            '    xlsheet.Cells(23, 9) = DS_DO.Tables(0).Rows(1)(2)
            'End If
            'If DS_DO.Tables(0).Rows(2)(1) > 0 Then
            '    xlsheet.Cells(26, 6) = DS_DO.Tables(0).Rows(2)(1)
            'End If
            'If DS_DO.Tables(0).Rows(2)(2) > 0 Then
            '    xlsheet.Cells(26, 9) = DS_DO.Tables(0).Rows(2)(2)
            'End If
            'If DS_DO.Tables(0).Rows(3)(1) > 0 Then
            '    xlsheet.Cells(27, 6) = DS_DO.Tables(0).Rows(3)(1)
            'End If
            'If DS_DO.Tables(0).Rows(3)(2) > 0 Then
            '    xlsheet.Cells(27, 9) = DS_DO.Tables(0).Rows(3)(2)
            'End If
            'If DS_DO.Tables(0).Rows(4)(1) > 0 Then
            '    xlsheet.Cells(28, 6) = DS_DO.Tables(0).Rows(4)(1)
            'End If
            'If DS_DO.Tables(0).Rows(4)(2) > 0 Then
            '    xlsheet.Cells(28, 9) = DS_DO.Tables(0).Rows(4)(2)
            'End If
    ...
    Private Sub china()
        ...
        Try
            ...
            xlsheet.Cells(11, 6) = DS_DO.Tables(0).Rows(0)(1)
            xlsheet.Cells(11, 8) = DS_DO.Tables(0).Rows(0)(2)

            xlsheet.Cells(14, 6) = DS_DO.Tables(0).Rows(1)(1)
            xlsheet.Cells(14, 8) = DS_DO.Tables(0).Rows(1)(2)

            xlsheet.Cells(17, 6) = DS_DO.Tables(0).Rows(2)(1)
            xlsheet.Cells(17, 8) = DS_DO.Tables(0).Rows(2)(2)
            xlsheet.Cells(18, 6) = DS_DO.Tables(0).Rows(3)(1)
            xlsheet.Cells(18, 8) = DS_DO.Tables(0).Rows(3)(2)

            xlsheet.Cells(19, 6) = DS_DO.Tables(0).Rows(4)(1)
            xlsheet.Cells(19, 8) = DS_DO.Tables(0).Rows(4)(2)
    ...
    Private Sub eng()
        ...
        Try
            ...
            xlapp = CType(CreateObject("excel.application"), Excel.Application)
            xlbook = CType(xlapp.Workbooks.Open("" & Filepath & ""), Excel.Workbook)
            xlsheet = CType(xlbook.Worksheets("理货结果汇总证明英文版"), Excel.Worksheet)
            xlapp.DisplayAlerts = False
            xlsheet.Application.Visible = True
