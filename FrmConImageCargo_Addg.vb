                ...
                If Trim(Me.txtBayNo.Text) Like "######" Then
                    ...
                    If Filldata(sqlUnique).Count = 0 Then
                        ...
                        If iBeingCount >= 1 Or iConCheck = 2 Or iConCheck = 1 Then   '箱号可能通过检验
                            ...
                            'If MessageBox.Show("是否增加下一条？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                            ...
                            Me.Close()
                        Else
                            MessageBox.Show("该集装箱号不规范，不能增加", "错误提示", MessageBoxButtons.OK, MessageBoxIcon.Stop)
                        End If
                    Else
                        MessageBox.Show("同一船次 集装箱号不可重复 贝位号不可重复（包括通贝）", "错误提示", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End If
                Else
