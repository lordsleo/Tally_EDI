...
Err:
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        Dim sqlNameExist As String
        Dim dvNameExist As New DataView
        Try
            Dim dsSave As New DataSet
            If Len(Trim(txtCode.Text)) > 0 And Len(Trim(txtCHA.Text)) > 0 Then
                If Getdata("select * from code_ShipName where SPCode='" & Trim(txtCode.Text).ToUpper & "'", dsSave).Count > 0 Then
                    MsgBox("已存在此船码信息,请注意检查!")
                    Exit Sub
                End If
                'sqlNameExist = "select * from code_ShipName where CHI_VESSEL='" & Trim(txtCHA.Text).ToUpper & "'"
                'dvNameExist = Filldata(sqlNameExist)
                'If dvNameExist.Count = 0 Then

                'Else
                '    Me.txtCHA.Focus()
                '    MessageBox.Show("中文船名不可重复", "提示")
                'End If
                Dim row As DataRow
                row = ds.Tables(0).NewRow()

                row("SPCode") = Trim(txtCode.Text).ToUpper
                row("CHI_VESSEL") = Trim(txtCHA.Text).ToUpper
                row("ENG_VESSEL") = Trim(txtENG.Text).ToUpper

                ds.Tables(0).Rows.Add(row)
                sqlda.Update(ds)
                Me.Close()
            Else
                MsgBox("船码 中文船名 不能为空!", MsgBoxStyle.Critical, "提示")
            End If
