...
Public Class FrmReadMailList_T
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    Friend WithEvents LShipCode As System.Windows.Forms.Label
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Me.LShipCode = New System.Windows.Forms.Label
        ...
        'LShipCode
        '
        Me.LShipCode.Location = New System.Drawing.Point(242, 24)
        Me.LShipCode.Name = "LShipCode"
        Me.LShipCode.Size = New System.Drawing.Size(128, 14)
        Me.LShipCode.TabIndex = 26
        '
        'Label2
        '
...
Err:
    ...
    Private Sub ReadZH()
        ...
        Else
            ...
            End If
        End If
        If ShipCode <> LShipCode.Text.Trim() Or LVoyage.Text.Trim() <> Voyage Then
            MsgBox("报文船码 航次 与 所选 船舶的 船码 航次 不符 不能 处理! ")
            Exit Sub
        End If
        '*************处理记录 11 描述船舶有关的补充信息
        ...
        Do While j < 9999
                ...
                Do While 1 = 1
                    ...
                    If Trim(str) = "51" Then
                        ...
                        If i99 = 1 Then
                            ...
                            '    Frm.ShowDialog()
                            '    ConType = Trim(G_SelectCode)
                            '    If Len(Trim(ConType)) = 4 Then
                            '        SSSS = Mid(ConType, 1, 2)
                            '        Type = Mid(ConType, 3, 2)
                            '    End If
                            'End If
                            ''*****************写入箱信息(箱规范)
                            ...
                            If dw.Count > 0 Then
                                ...
                                ds.Tables(0).Rows(0).Item("TempUnit") = ConTempUnit
                                If Len(Trim(ConTempSet)) > 0 And IsNumeric(ConTempSet) Then
                                    ds.Tables(0).Rows(0).Item("ReeferTemp") = CType(ConTempSet, Double)
                                End If
                                ds.Tables(0).Rows(0).Item("DangerGrade") = DangerousClass
                                ds.Tables(0).Rows(0).Item("DangerMark") = DangerousLabel
                            ...
                            '    'ds.Tables(0).Rows(0).Item("CONTAINER_NO") = Trim(ConNo)
                            '    'ds.Tables(0).Rows(0).Item("Mark") = BMark
                            '    'row("BLNO") = BLNO
                            '    'If Len(Trim(ConAmount)) > 0 Then
            ...
            If i99 = 1 Then
                ...
                If dw.Count > 0 Then
                    ...
                    'row("LOAD_PORT") = LoadPort
                    ds.Tables(0).Rows(0).Item("CODE_UnLOAD_PORT") = unLoadPortCode
                    ds.Tables(0).Rows(0).Item("UnLOAD_PORT") = unLoadPort
                    ds.Tables(0).Rows(0).Item("CODE_PLACE_RECEIVE") = DeliveryPlaceCode
                    ds.Tables(0).Rows(0).Item("PLACE_RECEIVE") = DeliveryPlace
                    ds.Tables(0).Rows(0).Item("CONSIGNMENT_ITEM") = DeliveryTerm

                    If Len(Trim(ConTempSet)) > 0 And IsNumeric(ConTempSet) Then
                        ds.Tables(0).Rows(0).Item("REFRIGERATE_TEMPERATURE") = CType(ConTempSet, Double)
                    End If
                    ds.Tables(0).Rows(0).Item("TEMPERATURE") = ConTempUnit
                    ds.Tables(0).Rows(0).Item("DANGER_GRADE") = DangerousClass
                    ds.Tables(0).Rows(0).Item("DANGER_NO") = DangerousPage
                    ds.Tables(0).Rows(0).Item("DANGER_PAGENO") = DangerousUNNO
                    ds.Tables(0).Rows(0).Item("DANGER_MARK") = DangerousLabel
