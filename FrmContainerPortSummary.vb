...
Public Class FrmContainerPortSummary
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmContainerPortSummary))
...
#End Region
    ...
    Private Sub FrmContainerPortSummary_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub C1DBGV_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles C1DBGV.DoubleClick
        ...
        FirmStr = " and Ship_ID = " & Ship_ID & " order by UnloadPort,Container_NO,BillNo "
        ...
        Call preview("WL3_1_19_1") '*****************权限设计 '*************要修改模块号
...
Err:
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        'Try
        '    '不显示的列名在最前面
        '    strsql = "select " & SelStr & " *  from View_ContainerNoList  where " & Sql '******要修改
...
Err:
    ...
    Private Sub GetSheetExcel()
        ...
        sqlStateSumary = "SELECT CODE_UNLOAD_PORT, MAX(UNLOAD_PORT) AS UnloadPort, FULLOREMPTY, SIZE_CON, COUNT(*) AS ConNum, SUM(AMOUNT) AS ConQuantity, SUM(WEIGHT) AS ConWeight, SUM(VOLUME) AS Size FROM View_ContainerLoadingSumary WHERE (CODE_LOAD_PORT <> 'LYG') GROUP BY CODE_UNLOAD_PORT, FULLOREMPTY, SIZE_CON ORDER BY CODE_UNLOAD_PORT, FULLOREMPTY, SIZE_CON"
        ...
        Try
            ...
            For Row = 0 To dvStateSumary.Count - 1
                ...
                If strPortNowCode <> strPortPreCode Then '如果前后港口不同了
                    '先填充数据再重新初始化
                    xlSheet.Cells(6 + 2 * i, 1) = strPort '填充数据
                    xlSheet.Cells(6 + 2 * i, 2) = iAmountH20
                    xlSheet.Cells(6 + 2 * i, 3) = iAmountH40
                    xlSheet.Cells(6 + 2 * i, 4) = iAmountH45
                    xlSheet.Cells(6 + 2 * i, 5) = iAmountH58
                    xlSheet.Cells(6 + 2 * i, 6) = iAmountE20
                    xlSheet.Cells(6 + 2 * i, 7) = iAmountE40
                    xlSheet.Cells(6 + 2 * i, 8) = iAmountE45
                    xlSheet.Cells(6 + 2 * i, 9) = iAmountE58

                    xlSheet.Cells(6 + 2 * i, 10) = iQuantityEFL
                    xlSheet.Cells(6 + 2 * i, 11) = fWeightEFL
                    xlSheet.Cells(6 + 2 * i, 12) = fSizeEFL
                    xlSheet.Cells(6 + 2 * i, 13) = iAmountH20 + iAmountH40 * 2 + iAmountH45 * 2.5 + iAmountH58 * 3 + iAmountE20 + iAmountE40 * 2 + iAmountE45 * 2.5 + iAmountE58 * 3
            ...
            If dvStateSumary.Count > 0 Then
                xlSheet.Cells(6 + 2 * i, 1) = strPort '填充最后一项数据
                xlSheet.Cells(6 + 2 * i, 2) = iAmountH20
                xlSheet.Cells(6 + 2 * i, 3) = iAmountH40
                xlSheet.Cells(6 + 2 * i, 4) = iAmountH45
                xlSheet.Cells(6 + 2 * i, 5) = iAmountH58
            ...
            xlSheet.Cells(7 + 2 * i, 1) = "合计" '填充总计数据
            xlSheet.Cells(7 + 2 * i, 2) = iAmountH20T
            xlSheet.Cells(7 + 2 * i, 3) = iAmountH40T
            xlSheet.Cells(7 + 2 * i, 4) = iAmountH45T
            xlSheet.Cells(7 + 2 * i, 5) = iAmountH58T
            xlSheet.Cells(7 + 2 * i, 6) = iAmountE20T
            xlSheet.Cells(7 + 2 * i, 7) = iAmountE40T
            xlSheet.Cells(7 + 2 * i, 8) = iAmountE45T
            xlSheet.Cells(7 + 2 * i, 9) = iAmountE58T
            xlSheet.Cells(7 + 2 * i, 10) = iQuantityT
            xlSheet.Cells(7 + 2 * i, 11) = fWeightT
            xlSheet.Cells(7 + 2 * i, 12) = fSizeT
            xlSheet.Cells(7 + 2 * i, 13) = iAmountH20T + iAmountH40T * 2 + iAmountH45T * 2.5 + iAmountH58T * 3 + iAmountE20T + iAmountE40T * 2 + iAmountE45T * 2.5 + iAmountE58T * 3
