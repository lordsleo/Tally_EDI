...
Public Class Frm_con_edi_stat_month
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(Frm_con_edi_stat_month))
        ...
        Me.c1dbg.Size = New System.Drawing.Size(676, 497)
        Me.c1dbg.TabIndex = 0
        ...
        'btn_enter
        '
        Me.btn_enter.Location = New System.Drawing.Point(264, 14)
        ...
        'btn_cancle
        '
        Me.btn_cancle.Location = New System.Drawing.Point(346, 14)
        ...
        Me.dt_month.Location = New System.Drawing.Point(167, 16)
        ...
        'btn_export
        '
        Me.btn_export.Location = New System.Drawing.Point(428, 14)
        ...
        Me.Name = "Frm_con_edi_stat_month"
...
#End Region
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_enter.Click
        Me.c1dbg.DataSource = Getdata("sp_con_edi_stat_month  '" & Me.dt_month.Value & "'", ds)
        caption()
        columnswidth()
    End Sub
    ...
    Private Sub caption()
        ...
        Me.c1dbg.Columns("ship_count").Caption = "件杂货_艘次"
        Me.c1dbg.Columns("ship_count_sum").Caption = "艘次累计"
        Me.c1dbg.Columns("teu").Caption = "件杂货_吨数"
        Me.c1dbg.Columns("teu_sum").Caption = "吨数累计"
        Me.c1dbg.Columns("income").Caption = "件杂货_收入"
        Me.c1dbg.Columns("income_sum").Caption = "收入累计"
        Me.c1dbg.Columns("remark").Caption = "备注"
        '合计
        Dim i, j, num, m As Integer
        i = 1
        Me.c1dbg.ColumnFooters = True
        Me.c1dbg.FooterStyle.BackColor = System.Drawing.Color.CadetBlue
        Me.c1dbg.Columns.Item("SHIPAGENT_CHA").FooterText = "合  计"
        Do While i < Me.c1dbg.Columns.Count - 1
            j = 0
            For j = 0 To ds.Tables(0).Rows.Count - 1
                num = 0
                For m = 0 To ds.Tables(0).Rows.Count - 1
                    If ds.Tables(0).Rows(m)(i) Is System.DBNull.Value Then
                    Else
    ...
    Private Sub MakeExcel()
        ...
        Try
            ...
            xlSheet = CType(xlBook.Worksheets("con_edi_stat_month"), Excel.Worksheet)
            xlApp.DisplayAlerts = False
            xlSheet.Application.Visible = True
            xlBook.Worksheets("con_edi_stat_month").select()
            '标题
            'xlSheet.Range(xlSheet.Cells(1, 1), xlSheet.Cells(2, 10)).Merge()
            'xlSheet.Cells(1, 1) = "外理连云港东达实业公司出口集装箱EDI月度统计报表"

            '报表月份
            xlSheet.Cells(3, 4) = DatePart(DateInterval.Year, Me.dt_month.Value) & " 年 " & DatePart(DateInterval.Month, Me.dt_month.Value) & " 月份"
            '制单日期
            xlSheet.Cells(3, 7) = Today()

            Me.c1dbg.MoveFirst()
            For Row = 0 To ds.Tables(0).Rows.Count - 1 '数据
                '写序号
                xlSheet.Cells(Row + 5, 1) = Row + 1
                For Col = 0 To Me.c1dbg.Columns.Count - 1
                    xlSheet.Cells(Row + 5, Col + 1) = Me.c1dbg.Columns(Col).Text.ToString     'ds.Tables(0).Rows(Row)(Col)
                Next Col
                Me.c1dbg.MoveNext()
            Next Row
            '合计
            For Col = 0 To Me.c1dbg.Columns.Count - 1 '合计
                xlSheet.Cells(ds.Tables(0).Rows.Count + 5, Col + 1) = Me.c1dbg.Columns.Item(Col).FooterText
            Next
            For Row = 5 To ds.Tables(0).Rows.Count + 5 '画列
            ...
            Next Row
            For Col = 1 To c1dbg.Columns.Count + 1 '画行
                xlSheet.Range(xlSheet.Cells(5, Col), xlSheet.Cells(ds.Tables(0).Rows.Count + 5, Col)).Borders(Excel.XlBordersIndex.xlEdgeLeft).LineStyle = 7
            Next Col
        Catch
            xlApp.Quit()
            SendKeys.Send("N")
