...
Public Class FrmConImageCargo_Edit
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.btQuit.Location = New System.Drawing.Point(322, 346)
        ...
        'btSave
        '
        Me.btSave.Location = New System.Drawing.Point(254, 346)
        Me.btSave.Name = "btSave"
        Me.btSave.Size = New System.Drawing.Size(65, 24)
        Me.btSave.TabIndex = 459
        Me.btSave.Text = "确认"
        '
        'cbGoodsCode
        '
...
#End Region
    ...
    Private Sub FrmConImageCargo_Edit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        ...
        Try
            If Trim(Me.txtContainerNo.Text) <> "" And Trim(Me.txtBayNo.Text) <> "" And Me.cbState.SelectedValue <> "0" Then
                ...
                If Me.SetNumValue("CONTAINER", txtContainerSize.Text, "箱尺寸") = False Then
                    txtContainerSize.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("AMOUNT", txtAmount.Text, "件数") = False Then
                    txtAmount.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("GROSSWEIGHT", txtGROSSWEIGHT.Text, "箱毛重") = False Then
                    Me.txtGROSSWEIGHT.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("TAREWEIGHT", txtTAREWEIGHT.Text, "箱皮重") = False Then
                ...
                ElseIf Me.SetNumValue("MAX_TEMPERATURE", txtTempMin.Text, "冷藏最低温度") = False Then
                    Me.txtTempMin.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("OVER_HEIGHT", txtOverHeight.Text, "超高") = False Then
                    Me.txtOverHeight.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("OVER_LEFT", txtOverLeft.Text, "左超") = False Then
                    Me.txtOverLeft.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("OVER_RIGHT", txtOverRight.Text, "右超") = False Then
                    Me.txtOverRight.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("OVER_BEFORE", txtOverBefore.Text, "前超") = False Then
                    Me.txtOverBefore.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("OVER_BEHIND", txtOverBehind.Text, "后超") = False Then
                    Me.txtOverBehind.Focus()
                    Exit Sub
                ElseIf Me.SetNumValue("DANGER_PAGENO", txtDANGER_PAGENO.Text, "危险品页号") = False Then
                    Me.txtDANGER_PAGENO.Focus()
                    Exit Sub
                End If
                ds.Tables(0).Rows(0).Item("CONTAINER_NO") = Trim(Me.txtContainerNo.Text)
                ds.Tables(0).Rows(0).Item("SEALNO") = Trim(Me.txtSealNo.Text)
                ds.Tables(0).Rows(0).Item("BAYNO") = Trim(Me.txtBayNo.Text)
                ds.Tables(0).Rows(0).Item("BLNO") = Trim(Me.txtBillNO.Text)
                ds.Tables(0).Rows(0).Item("MARK") = Trim(Me.txtMark.Text)
                ds.Tables(0).Rows(0).Item("GOODS_ENG") = Trim(Me.txtGOODS_ENG.Text)
                ...
                ds.Tables(0).Rows(0).Item("MANUFACTURER") = Trim(Me.txtCompany.Text)
                ds.Tables(0).Rows(0).Item("KEEPER") = Trim(Me.txtKeeper.Text)

                ds.Tables(0).Rows(0).Item("FULLOREMPTY") = Me.cbState.SelectedValue
                ds.Tables(0).Rows(0).Item("CODE_GOODS") = Me.cbGoodsCode.SelectedValue
                ds.Tables(0).Rows(0).Item("CODE_LOAD_PORT") = Me.cbLoadPortCode.SelectedValue
                ds.Tables(0).Rows(0).Item("CODE_UNLOAD_PORT") = Me.cbUnloadPortCode.SelectedValue
                ds.Tables(0).Rows(0).Item("CODE_DELIVERY") = Me.cbDeliveryCode.SelectedValue
                ds.Tables(0).Rows(0).Item("TEMPERATURE_UNIT") = Trim(Me.cbTempUnit.Text)
                ds.Tables(0).Rows(0).Item("CODE_MANUFACTURER") = Trim(Me.cbCompanyCode.Text)
                ds.Tables(0).Rows(0).Item("CODE_KEEPER") = Trim(Me.cbKeeperCode.Text)
                ds.Tables(0).Rows(0).Item("PASS_MARK") = Trim(Me.cbPassMark.Text)
                ds.Tables(0).Rows(0).Item("SHORT_UNLOAD") = Trim(Me.cbShortUnload.Text)
                ...
                If MessageBox.Show("确定对进口船图的修改吗？", "警告提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
                    sqlUnique = "SELECT * FROM CON_IMAGE_RECORD WHERE SHIP_ID = '" & Ship_ID & "' AND ( CONTAINER_NO = '" & Trim(Me.txtContainerNo.Text) & "' or BAYNO = '" & Trim(Me.txtBayNo.Text) & "' ) and CON_IMAGE_RECORD_ID <> '" & ID & "'"
