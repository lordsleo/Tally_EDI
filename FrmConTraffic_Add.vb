...
Public Class FrmConTraffic_Add
...
Err:
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        Try
            ...
            If Getdata("select * from Con_TRAFFIC_ITEM where Con_TRAFFIC_ITEM='" & Trim(txtCode.Text) & "'", dsSave).Count > 0 Then
                MsgBox("可能已存在此运输条款信息,请注意检查!")
                Exit Sub
            End If
            If Len(Trim(txtCode.Text)) > 0 And Len(Trim(txtCHA.Text)) > 0 Then
                ...
                row("Con_TRAFFIC_ITEM") = Trim(txtCode.Text)
                row("CON_TRAFFIC_ITEM_CHA") = Trim(txtCHA.Text)
                row("CON_TRAFFIC_ITEM_ENG") = Trim(txtENG.Text)

                ds.Tables(0).Rows.Add(row)
                sqlda.Update(ds)
                Me.Close()
            Else
                MsgBox("条款代码不能为空!", MsgBoxStyle.Critical, "提示")
            End If
