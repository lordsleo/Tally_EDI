...
#End Region
    ...
    Private Sub btResure_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btResure.Click
        ...
        '******************理货部
        '集装箱增加EDI班 ，陶明月021045，吴炎020908，许涛190853，邱杰184568，成杰111008
        ...
        sqlstr = "select * from ABSENT where (DEPT_CODE = '" & G_DeptCode & "' or  DEPT_CODE ='29." & Mid(Trim(G_DeptCode), 4) & "' )  and  datediff(dd,ABSENTDATE,'" & GL_Time & "')=0 and work_no in ('021045','020908','190853','111008') ORDER BY DEPT_CODE ,Code_HOLIDAY_TYPE "
        '
        'MsgBox(sqlstr)
        dwkq = Updatedata(sqldakq, sqlstr, dskq)
        ...
        If dwkq.Count = 0 Then
            ...
            If MsgBox("当班考勤还没有生成，要生成当班考勤记录吗？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.Yes Then
                ...
                execsqlstr = execsqlstr & "Code_HOLIDAY_Back, DAYNIGHTMARK, Code_workstat,USER_NAME) "
                execsqlstr = execsqlstr & "select dept_code,work_no,'" & GL_Time & "', "
                execsqlstr = execsqlstr & "'EDI', '11', '11','01','02','" & G_User & "' from tally_clerk "
                execsqlstr = execsqlstr & " where (dept_code='" & G_DeptCode & "' or DEPT_CODE ='29." & Mid(Trim(G_DeptCode), 4) & "') and work_no in ('021045','020908','190853','111008')  "
