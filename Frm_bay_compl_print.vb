...
Public Class Frm_bay_compl_print
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'btn_all
        '
        Me.btn_all.Location = New System.Drawing.Point(440, 42)
        Me.btn_all.Name = "btn_all"
        Me.btn_all.Size = New System.Drawing.Size(64, 24)
        Me.btn_all.TabIndex = 16
        Me.btn_all.Text = "全船打印"
        '
        'Label10
        '
        Me.Label10.Location = New System.Drawing.Point(86, 44)
        ...
        'txt_scale_rate
        '
        Me.txt_scale_rate.Location = New System.Drawing.Point(52, 46)
        ...
        'btn_printpreview
        '
        Me.btn_printpreview.Location = New System.Drawing.Point(294, 42)
        ...
        'btn_pagesetup
        '
        Me.btn_pagesetup.Location = New System.Drawing.Point(222, 42)
        ...
        'btn_enter
        '
        Me.btn_enter.Location = New System.Drawing.Point(368, 42)
        ...
        'cmb_defined_bay
        '
        Me.cmb_defined_bay.Location = New System.Drawing.Point(144, 46)
        ...
        'btn_cancle
        '
        Me.btn_cancle.Location = New System.Drawing.Point(512, 42)
        ...
        Me.Pl_bay_map.Size = New System.Drawing.Size(572, 673)
        ...
        Me.GroupBox2.Location = New System.Drawing.Point(572, 0)
        Me.GroupBox2.Name = "GroupBox2"
        Me.GroupBox2.Size = New System.Drawing.Size(208, 747)
...
#End Region
    ...
    Private Sub Frm_bay_compl_print_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        topsize = 5
        notshowbaymap = 0
    ...
    Private Sub cmblygbaylist()
        '写已存在的贝列_全船
        If Getdata("sp_get_bay " & Ship_ID & ",'" & GL_IOPORT & "' ", ds_exist_bay).Count > 0 Then
            For i = 0 To ds_exist_bay.Tables(0).Rows.Count - 1
                Me.cmb_defined_bay.Items.Add(ds_exist_bay.Tables(0).Rows(i).Item("bay_num"))
            Next
        End If
            ...
            For i = small_row To big_row
                For j = small_col To big_col
                    If ds_board.Tables(0).Rows((big_col - small_col + 1) * (i - small_row) + j - small_col).Item("user_char") = 1 Then
                        ...
                        ' If BAYNO = "371290" Or BAYNO = "381290" Then
                        ' M() 'essageBox.Show(BAYNO)
                        ' End If
                        '判断此贝位号有无箱号???????????????????????????????????????????????????????????????????????????????????????????????????????
                        If Getdata("select con_image.CONTAINER_NO,con_unload_port_substitute.substitute  from con_image left join con_unload_port_substitute on con_image.ship_id=con_unload_port_substitute.ship_id and right(con_image.code_unload_port,3)=right(con_unload_port_substitute.code_unload_port,3)  where   con_image.BAYNO='" & BAYNO & "'and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
                            '判断卸货港应该用什么样的颜色
                        ...
                        If Getdata("select * from CON_BAY where v_id='" & v_id & "' and bay_num='" & Trim(Me.cmb_defined_bay.Text) & "' and joint='1'", dsjoint).Count > 0 Then
                            ...
                            '判断此通贝贝位号有无箱号
                            If Getdata("select con_image.CONTAINER_NO,con_unload_port_substitute.substitute  from con_image left join con_unload_port_substitute on con_image.ship_id=con_unload_port_substitute.ship_id and right(con_image.code_unload_port,3)=right(con_unload_port_substitute.code_unload_port,3)   where  con_image.BAYNO='" & bayno_joint & "'and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
                                '判断卸货港应该用什么样的颜色
                                fill_rectangle_color()
                                '填充矩形
                                bmpgraphics_bay_map.FillRectangle(bru_fill, distolefe + col_count * 10 + 20 * (j - small_col + 1) + 1, distotop + board_height + 20 - 20 * (i - small_row + 1) + 1 + 20, 19, 19)
                        ...
                        If Getdata("select * from CON_BAY where v_id='" & v_id & "' and bay_num='" & BAYNO_MID_1 & "' and joint='1'", dsjoint).Count > 0 Then
                            ...
                            If Len(Trim(BAYNO_MID)) > 0 Then
                                If Getdata("select substring(ltrim(CODE_UNLOAD_PORT),1,1) CODE_UNLOAD_PORT,FULLOREMPTY,substring(ltrim(CONTAINER_TYPE),1,1) CONTAINER_TYPE from con_image left outer join CON_CRITERION on con_image.container_No=CON_CRITERION.container_No  where  con_image.BAYNO='" & BAYNO_MID & "' and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
            ...
            For i = small_row To big_row
                ...
                For j = small_col To big_col
                    If ds_board.Tables(0).Rows((big_col - small_col + 1) * (i - small_row) + j - small_col).Item("user_char") = 1 Then
                        ...
                        End If
                        '判断此贝位号及通贝的贝位号上有无箱号
                        If Getdata("select con_image.CONTAINER_NO,right(CODE_LOAD_PORT,3) CODE_LOAD_PORT,right(CODE_UNLOAD_PORT,3) CODE_UNLOAD_PORT,right(CODE_DELIVERY,3) CODE_DELIVERY ,cast(cast(round(GROSSWEIGHT/1000,2) as numeric(9,2)) as varchar) as GROSSWEIGHT,SIZE_CON,CONTAINER_TYPE,FULLOREMPTY,TEMPERATURE_UNIT,TEMPERATURE_SETTING,MIN_TEMPERATURE,MAX_TEMPERATURE,DANGER_GRADE from con_image left outer join CON_CRITERION on con_image.container_No=CON_CRITERION.container_No  where  (con_image.BAYNO='" & BAYNO & "' or con_image.BAYNO='" & bayno_joint & "')  and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
                            '装货钢
                        ...
                        End If
                        ''写贝位号
                        'If Getdata("select con_image.CONTAINER_NO,substring(ltrim(CODE_LOAD_PORT),1,3) CODE_LOAD_PORT,substring(ltrim(CODE_UNLOAD_PORT),1,3) CODE_UNLOAD_PORT,CODE_DELIVERY,GROSSWEIGHT,SIZE_CON,CONTAINER_TYPE from con_image left outer join CON_CRITERION on con_image.container_No=CON_CRITERION.container_No  where  (con_image.BAYNO='" & bayno_joint & "')  and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
    ...
    Private Sub Pl_bay_map_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles Pl_bay_map.MouseDown
        ...
        If color1.ToArgb <> Color.White.ToArgb Then ' color1.ToArgb = Color.Blue.ToArgb Or color1.ToArgb = Color.DarkGray.ToArgb
            '计算甲板结果图的顶点位置
            If (big_col_cabin - small_col_cabin + 1) - (big_col - small_col + 1) >= 0 Then '比较甲板和舱内的列数
            ...
            If Getdata(bayno_check, ds_bayno).Count > 0 Then
                ...
                End If
                '贝位号
                If Getdata("select con_image.CONTAINER_NO,CODE_LOAD_PORT,CODE_UNLOAD_PORT,CODE_DELIVERY,GROSSWEIGHT,SIZE_CON,CONTAINER_TYPE from con_image left outer join CON_CRITERION on con_image.container_No=CON_CRITERION.container_No  where ( con_image.BAYNO='" & bayno_check & "') and con_image.ship_id=" & Ship_ID, ds_container).Count > 0 Then
                    '贝位号
