...
Public Class FrmConLoadCheck_TongDeal
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmConLoadCheck_TongDeal))
        ...
        Me.btnCommit.Location = New System.Drawing.Point(270, 462)
        ...
        Me.btQuit.Location = New System.Drawing.Point(340, 462)
        ...
        Me.C1DBG.Size = New System.Drawing.Size(674, 426)
        Me.C1DBG.TabIndex = 4
        ...
        ", 0, 670, 422</ClientRect><BorderSide>0</BorderSide><BorderStyle>Sunken</BorderS" & _
        ...
        "</Layout><DefaultRecSelWidth>16</DefaultRecSelWidth><ClientArea>0, 0, 670, 422</" & _
        "ClientArea><PrintPageHeaderStyle parent="""" me=""Style14"" /><PrintPageFooterStyle " & _
        "parent="""" me=""Style15"" /></Blob>"
        '
        'txtBillNO
        '
        Me.txtBillNO.Location = New System.Drawing.Point(546, 6)
        Me.txtBillNO.MaxLength = 120
        Me.txtBillNO.Name = "txtBillNO"
        Me.txtBillNO.Size = New System.Drawing.Size(114, 21)
        ...
        Me.Label4.Location = New System.Drawing.Point(488, 8)
        ...
        Me.txtShipName.Location = New System.Drawing.Point(330, 6)
        Me.txtShipName.MaxLength = 40
        Me.txtShipName.Name = "txtShipName"
        Me.txtShipName.ReadOnly = True
        Me.txtShipName.Size = New System.Drawing.Size(152, 21)
        ...
        'txtVoyage
        '
        Me.txtVoyage.Location = New System.Drawing.Point(196, 6)
        Me.txtVoyage.MaxLength = 6
        Me.txtVoyage.Name = "txtVoyage"
        Me.txtVoyage.Size = New System.Drawing.Size(96, 21)
        ...
        'txtShip
        '
        Me.txtShip.Location = New System.Drawing.Point(62, 6)
        Me.txtShip.MaxLength = 40
        Me.txtShip.Name = "txtShip"
        Me.txtShip.Size = New System.Drawing.Size(96, 21)
        ...
        Me.lbShow.BackColor = System.Drawing.Color.Lavender
        Me.lbShow.FlatStyle = System.Windows.Forms.FlatStyle.Flat
        ...
        Me.lbShow.ForeColor = System.Drawing.SystemColors.ActiveCaption
        Me.lbShow.Location = New System.Drawing.Point(2, 460)
        Me.lbShow.Name = "lbShow"
        Me.lbShow.Size = New System.Drawing.Size(242, 23)
        Me.lbShow.TabIndex = 352
        Me.lbShow.TextAlign = System.Drawing.ContentAlignment.MiddleLeft
        '
        'FrmConLoadCheck_TongDeal
        '
        ...
        Me.Name = "FrmConLoadCheck_TongDeal"
...
#End Region
    ...
    Private Sub FrmConLoadCheck_TongDeal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub C1DBG_AfterColEdit(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.ColEventArgs) Handles C1DBG.AfterColEdit
        ...
        If e.ColIndex = 4 Then
            Me.C1DBG.Columns("CONTAINER_NO").Text = Me.C1DBG.Columns("CONTAINER_NO").Text.Trim.ToUpper
            strContainerNo = Me.C1DBG.Columns("CONTAINER_NO").Text
            'sqlExist检查条件要改 加船码航次提单的限制 此处不加 ，最后保存时加（针对场站和检查桥过的拼箱，不加是将箱信息带出，加是为了增加此拼箱记录）
            sqlExist = "SELECT * FROM CON_LOAD_TALLY_LIST WHERE CHI_VESSEL = '" & Me.txtShip.Text & "' AND Voyage = '" & Me.txtVoyage.Text & "' AND BLNO = '" & Me.txtBillNO.Text & "' and (CONTAINER_NO = '" & strContainerNo & "' or CONTAINER_NO = '" & GetConOtherMode(strContainerNo) & "') and Checked = '0'"
    ...
    Private Sub TrimGrid()
        ...
        For i = 0 To dsContainer.Tables(0).Rows.Count - 1
            ...
            'End If
            If Me.txtShip.Text = "HCHE" And Me.txtVoyage.Text = "001" Then     '如果是火车箱001航次
                Me.C1DBG.Columns("StateC").Text = "0"       '状态为0
            End If
        Next
        Me.btQuit.Focus()
    ...
    Private Sub btnCommit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCommit.Click
        ...
        shipCode = Me.txtShip.Text
        Voyage = Me.txtVoyage.Text
        ...
        For i = 0 To dvContainer.Count - 1
            ...
            If containerNo <> "" Then
                ...
                '检查箱规范
                sqlExist = "SELECT * FROM CON_LOAD_TALLY_LIST WHERE CHI_VESSEL = '" & shipCode & "' AND Voyage = '" & Voyage & "' AND BLNO = '" & billNo & "' and (CONTAINER_NO = '" & containerNo & "' or CONTAINER_NO = '" & GetConOtherMode(containerNo) & "') and Checked = '0'"
                dvExist = Filldata(sqlExist)
                If dvExist.Count > 0 Then   '箱号已存在且未过关，验箱
                ...
                Else   '箱号不存在，检查箱，添加箱
                    ...
                    Else
                        ...
                        ElseIf iConCheck = 0 Then
                            ...
                            'MessageBox.Show("集装箱号不规范！", "提示")
                            'Exit For
                            If MessageBox.Show("集装箱号 " & containerNo & " 不存在于箱规范中且校验不正确！确定存在吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1) = DialogResult.No Then
                                Exit For
                            End If
                        End If
                    End If
                ...
                '检查重复
                For j = 0 To i - 1
                    If IsEqualConNo(containerNo, Me.C1DBG.Columns("CONTAINER_NO").CellText(j)) Then
                        Me.C1DBG.Row = i
                        Me.C1DBG.SelectedRows.Clear()
                        Me.C1DBG.SelectedRows.Add(i)
        ...
        If i >= dvContainer.Count Then    '界面检查无误
            For i = 0 To dvContainer.Count - 1    '数据库检查
                ...
                sqlExist = "SELECT CHI_VESSEL, Voyage, BLNO, CONTAINER_NO, Checked FROM CON_LOAD_TALLY_LIST WHERE CHI_VESSEL = '" & shipCode & "' AND Voyage = '" & Voyage & "' AND BLNO = '" & billNo & "' and (CONTAINER_NO = '" & containerNo & "' or CONTAINER_NO = '" & GetConOtherMode(containerNo) & "')"
                dvExist = Filldata(sqlExist)
                If dvExist.Count = 0 Then    '新加箱，没有重复 不管
                Else
                    If dvExist(0)("Checked") = "0" Then     '没有查验过,正常 不管 
                    Else
                        Me.C1DBG.Row = i
                        Me.C1DBG.SelectedRows().Clear()
                        Me.C1DBG.SelectedRows.Add(i)
        ...
        If i >= dvContainer.Count Then  '修改现有箱信息 
            For i = dvContainer.Count - 1 To 0 Step -1
                ...
                sqlExist = "SELECT CHI_VESSEL, Voyage, BLNO, CONTAINER_NO, Checked FROM CON_LOAD_TALLY_LIST WHERE CHI_VESSEL = '" & shipCode & "' AND Voyage = '" & Voyage & "' AND BLNO = '" & billNo & "' and (CONTAINER_NO = '" & containerNo & "' or CONTAINER_NO = '" & GetConOtherMode(containerNo) & "') and Checked = '0'"
                dvExist = Filldata(sqlExist)
        ...
        If i < 0 AndAlso Count > 0 Then   'i<0说明经过了修改箱信息过程 Count >0说明存在新增箱信息 可能需要增加提单 
            ...
            If dvBillNo.Count = 0 Then   '没有提单场站信息 加
                ...
                If shipCode = "HCHE" Then   '是火车大列的，状态为0
                    sqlInsertBill = "INSERT INTO CON_LOAD_TALLY (CHI_VESSEL, Voyage, InOutPort, CodeLoadPort, NO, WORK_PLACE, TIME_FROM, BLNO, WORKTYPE, CODE_TALLY_TYPE, USER_NAME, TALLY_CLERK, CODE_CON_CONSIGN, StateB) " & _
                            "VALUES ('" & shipCode & "', '" & Voyage & "', '1', 'CNLYG', '" & strNewNo & "', '" & strYard & "', '" & Now & "', '" & billNo & "', '2', '3', '" & G_User & "', '" & G_User & "', '00', '0')"
                Else
                    sqlInsertBill = "INSERT INTO CON_LOAD_TALLY (CHI_VESSEL, Voyage, InOutPort, CodeLoadPort, NO, WORK_PLACE, TIME_FROM, BLNO, WORKTYPE, CODE_TALLY_TYPE, USER_NAME, TALLY_CLERK, CODE_CON_CONSIGN, StateB) " & _
                            "VALUES ('" & shipCode & "', '" & Voyage & "', '1', 'CNLYG', '" & strNewNo & "', '" & strYard & "', '" & Now & "', '" & billNo & "', '2', '3', '" & G_User & "', '" & G_User & "', '00', '1')"
                End If
                ExecSql(sqlInsertBill)
            Else
                If dvBillNo(0)("StateB") >= 2 Then
                    MessageBox.Show("该提单已出证，不可处理", "提示")
                    Exit Sub
                End If
            End If
        End If
        ...
        Try
            If i < 0 Then  '全部通过检验且增加了该加的提单
                Me.C1DBG.Focus()
                For i = 0 To Count - 1
                    Me.C1DBG.Row = i
                    Me.C1DBG.Columns("CHI_VESSEL").Text = shipCode
                    Me.C1DBG.Columns("Voyage").Text = Voyage
                    Me.C1DBG.Columns("BLNO").Text = billNo
                Next
                Me.btQuit.Focus()
    ...
    Private Sub JudgeBill()
        sqlBillExist = "SELECT ShipName, Voyage, BLNO, Code_Yard_Name, StateB FROM View_ConLoadTallyReceipt WHERE CHI_VESSEL = '" & Me.txtShip.Text & "' AND Voyage = '" & Me.txtVoyage.Text & "' AND BLNO = '" & Me.txtBillNO.Text & "'"
        dvBillExist = Filldata(sqlBillExist)
        If dvBillExist.Count > 0 Then
            Me.lbShow.Text = "该提单已存在，属于" & dvBillExist(0)("Code_Yard_Name")
        Else
            Me.lbShow.Text = "新加提单"
        End If
    End Sub
