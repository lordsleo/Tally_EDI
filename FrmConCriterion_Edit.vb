...
Public Class FrmConCriterion_Edit
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.txtWeight.TabIndex = 196
        Me.txtWeight.Text = ""
        '
        'btQuit
        '
        Me.btQuit.Location = New System.Drawing.Point(209, 192)
        Me.btQuit.Name = "btQuit"
        Me.btQuit.Size = New System.Drawing.Size(65, 24)
        Me.btQuit.TabIndex = 194
        Me.btQuit.Text = "取消"
        '
        'btSave
        '
        Me.btSave.Location = New System.Drawing.Point(141, 192)
        Me.btSave.Name = "btSave"
        Me.btSave.Size = New System.Drawing.Size(65, 24)
        Me.btSave.TabIndex = 193
        Me.btSave.Text = "确认"
        '
        'txtHeight
        '
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        Try
            If MsgBox("你确认要修改吗？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.Yes Then
                Call OperateHistory(G_DeptName, Now, G_User, "修改", "CON_CRITERION", Trim(txtID.Text), "CON_CRITERION_ID")

                ds.Tables(0).Rows(0).Item("CONTAINER_NO") = txtNo.Text
                ds.Tables(0).Rows(0).Item("NATIONALITY_CODE") = cbNat.SelectedValue
                If Len(Trim(txtSize.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("SIZE_CON") = txtSize.Text
                End If
                ds.Tables(0).Rows(0).Item("CONTAINER_TYPE") = txtType.Text
                ds.Tables(0).Rows(0).Item("Code_Con_Company") = cbCon.SelectedValue
                ds.Tables(0).Rows(0).Item("CODE_SHIP_OWNER") = cbShip.SelectedValue
                If Len(Trim(txtWeight.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("CON_WEIGHT") = txtWeight.Text
                End If
                If Len(Trim(txtHeight.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("OVER_Height") = txtHeight.Text
                End If
                If Len(Trim(txtLeft.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("OVER_LEFT") = txtLeft.Text
                End If
                If Len(Trim(txtRight.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("OVER_RIGHT") = txtRight.Text
                End If
                If Len(Trim(txtBefore.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("OVER_BEFORE") = txtBefore.Text
                End If
                If Len(Trim(txtBehind.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("OVER_BEHIND") = txtBehind.Text
                End If
                If Len(Trim(txtSequence.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("SEQUENCE_NO") = txtSequence.Text
                End If
                If Len(Trim(txtAmount.Text)) > 0 Then
                    ds.Tables(0).Rows(0).Item("CHECK_AMOUNT") = txtAmount.Text
                End If
