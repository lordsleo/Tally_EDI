...
#End Region
    ...
    Private Sub Frm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FirmStr = " Order by SHIPAGENT_SHORT "  '" Use_Comp Like '" & Trim(G_DeptCode) & "%' and Hide_Company Like '" & Trim(G_Company) & "%' order by ID DESC" '设计初始条件  在检索后不改变的条件  '***************要修改 或 2>1
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Try
            ...
            Next
            Me.C1DBG.Columns("Mark").ValueItems.Presentation = C1.Win.C1TrueDBGrid.PresentationEnum.RadioButton
            'Me.C1DBG.Columns("Mark").ValueItems.Translate = True
            Dim vi As C1.Win.C1TrueDBGrid.ValueItem
            vi = New C1.Win.C1TrueDBGrid.ValueItem("1", "船代")
            C1DBG.Columns("Mark").ValueItems.Values.Add(vi)
            vi = New C1.Win.C1TrueDBGrid.ValueItem("0", "货代")
            C1DBG.Columns("Mark").ValueItems.Values.Add(vi)

            Call SetColumnCaption() '设计列标题
            Call SetColumnWidth() '设计列宽度
            C1DBG.Splits(0).DisplayColumns("Mark").Width += 50
