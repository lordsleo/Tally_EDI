    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        ...
        Try
            If MsgBox("确认要修改吗？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.Yes Then
                sqlExist = "select * from code_ShipName where ID <> '" & ID & "' and SPCode='" & Trim(txtCode.Text).ToUpper & "'"
                dvExist = Filldata(sqlExist)
                If dvExist.Count = 0 Then
                    'sqlNameExist = "select * from code_ShipName where ID <> '" & ID & "' and CHI_VESSEL='" & Trim(txtCHA.Text).ToUpper & "'"
                    ...
                    ExecSql("Exec sp_ConLoadTallyListOperPre '" & G_DeptName & "','" & Now & "','" & G_User & "','修改船码'")

                    ds.Tables(0).Rows(0).Item("SPCode") = Trim(txtCode.Text).ToUpper
